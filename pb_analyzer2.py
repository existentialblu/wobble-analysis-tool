...
def plot_periodic_segments(flow_data, sampling_rate, smoothed_abs_flow, peaks, troughs, periodic_segments_indices, filename, periodic_percentage, average_depth, average_wave_period):
    """
    Plots the smoothed flow, detected peaks/troughs, shades identified periodic segments,
    and includes key metrics as text on the plot.
    """
    if flow_data is None or sampling_rate is None or sampling_rate <= 0:
    ...
    # Prepare the text for the metrics
    metrics_text = (
        f"Periodic: {periodic_percentage:.2f}%\n"
        f"Depth: {average_depth:.2f}\n"
        f"Period: {average_wave_period:.2f} s"
    )
    
    # Add text box to the plot, adjusted to top-right corner
    plt.text(0.78, 0.98, metrics_text, transform=plt.gca().transAxes,
        ...
        if flow_data is not None and sampling_rate is not None and sampling_rate > 0:
            ...
            if dominant_period_sec is not None and dominant_period_sec != np.inf:
                ...
                if average_depth is not None and average_wave_period is not None:
                    ...
                    # Pass the new metrics to the plotting function
                    plot_periodic_segments(flow_data, sampling_rate, smoothed_abs_flow, peaks, troughs, periodic_segments_indices, filename, periodic_percentage, average_depth, average_wave_period)
